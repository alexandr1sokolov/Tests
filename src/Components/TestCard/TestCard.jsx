...
const TestCard = ({
  ...
  const addCurrentAnswersFunc = e => {
    ...
    let data;
    e.target.value === correctResult[e.target.dataset.index]
      ? (data = true)
      : (data = false);
    ...
    case "correct":
      return (
        <div className={`${styles.testcard} ${styles.testcardCor}`}>
          <p className={styles.testcard__question}>
            {+index + 1}. {question}
          </p>
          <form method="post" onChange={addCurrentAnswersFunc}>
            {answers.map((answ, i) => (
              <label
                className={`${styles.testcard__answer} ${checkedInd[index] ===
                  i && styles.marked__answer_correct}`}
                id={`${answ}${i}`}
                key={`${testname}${i}`}
              >
                <input
