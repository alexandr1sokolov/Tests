...
const Enter = props => {
  ...
  const post = () => {
    ...
    axios
      .post("https://student-tests.goit.co.ua/api/users/login", result)
      ...
      .then(() =>
        axios
          .get(
            `https://student-tests.goit.co.ua/api/users/${getUserId()}`,
            getUserAuthHeader()
          )
          .then(data => props.dataResultFunc(data.data.results))
      )
      .then(() => props.loginHandler())
      .catch(err => {
        console.log(err);
        props.setMessageTextFunc("Неправильный логин или пароль");
      });
  };
