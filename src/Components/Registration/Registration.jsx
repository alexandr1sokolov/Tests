...
const Registration = props => {
  ...
  const sumCheck = () => {
    if (valPass() && valMail()) {
      ...
      axios
        .post("https://student-tests.goit.co.ua/api/users", result)
        .then(result =>
          result.status === 201 ? props.successfulRegistrationFunc() : null
        )

        .catch(err => {
          console.log(err);
          props.setMessageTextFunc("Такой пользователь уже существует!");
        });
    }
  };
      ...
      {props.showAgreement ? (
        <Modal closeModal={closeRegModal}>
          ...
          <p className={styles.agreement}>
            Регестрируясь на сайте, я даю согласие на обработку моих
            персональных данных в соответствии с ЗУ"Про защиту персональных
            данных".
          </p>
        </Modal>
      ) : (
          ...
          <form className={styles.form} onSubmit={submit}>
            ...
            <div className={styles.lockCont}>
              ...
              <input
                type="password"
                pattern="[\w-]{6,10}$"
                title="Введите пароль от 6 до 10 символов"
                className={styles.input}
                placeholder="Password"
                value={props.passChange}
                ...
                <input
                  ...
                  onClick={
                    props.checkBoxStatus
                      ? props.checkBoxOffFunc
                      : props.checkBoxOnFunc
                  }
                />
              </label>
              Регистрируясь, вы принимаете{" "}
              <span className={styles.orangeSp} onClick={props.showAgr}>
